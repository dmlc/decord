...
def get_single_channel_reader():
    return AudioReader(os.path.join(os.path.dirname(__file__), '..', '..', 'cpp', 'audio', 'big_buck_bunny.mp4'), CTX)

def get_double_channels_reader():
    return AudioReader(os.path.join(os.path.dirname(__file__), '..', '..', 'cpp', 'audio', 'file_example_MOV_1280_1_4MB.mov'), CTX, mono=False)

def get_resampled_reader():
    return AudioReader(os.path.join(os.path.dirname(__file__), '..', '..', 'cpp', 'audio', 'big_buck_bunny.mp4'), CTX, 4410)

def get_channel_change_reader():
    return AudioReader(os.path.join(os.path.dirname(__file__), '..', '..', 'cpp', 'audio', 'file_example_MOV_1280_1_4MB.mov'), CTX)

def test_single_channel_audio_reader():
    ar = get_single_channel_reader()
    assert ar.shape == (1, 1326080)

def test_double_channels_audio_reader():
    ar = get_double_channels_reader()
    assert ar.shape == (2, 1465344)
