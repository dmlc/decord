...
from ..runtime_ctypes import DECORDType, DECORDContext, DECORDByteArray


cdef void decord_callback_finalize(void* fhandle):
    local_pyfunc = <object>(fhandle)
    Py_DECREF(local_pyfunc)

cdef int decord_callback(DECORDValue* args,
                      int* type_codes,
                      int num_args,
                      DECORDRetValueHandle ret,
                      void* fhandle) with gil:
    cdef list pyargs
    cdef DECORDValue value
    cdef int tcode
    local_pyfunc = <object>(fhandle)
    pyargs = []
    for i in range(num_args):
        ...
        if (tcode == kFuncHandle or
            ...
            tcode > kExtBegin):
            CALL(DECORDCbArgToReturn(&value, tcode))
    ...
    try:
        ...
        DECORDAPISetLastError(c_str(msg))
        return -1
    if rv is not None:
        ...
        CALL(DECORDCFuncSetReturn(ret, &value, &tcode, 1))
    return 0


def convert_to_decord_func(object pyfunc):
    ...
    Returns
    -------
    decordfunc: decord.Function
        The converted decord function.
    """
    cdef DECORDFunctionHandle chandle
    Py_INCREF(pyfunc)
    CALL(DECORDFuncCreateFromCFunc(decord_callback,
                                <void*>(pyfunc),
                                decord_callback_finalize,
                                &chandle))
...
cdef inline int make_arg(object arg,
    ...
    elif isinstance(arg, _DECORD_COMPATS):
        ptr = arg._decord_handle
        value[0].v_handle = (<void*>ptr)
        tcode[0] = arg.__class__._decord_tcode
...
cdef inline object make_ret(DECORDValue value, int tcode):
    ...
    elif tcode == kDECORDContext:
        return DECORDContext(value.v_ctx.device_type, value.v_ctx.device_id)
    # (minjie): Node and class module are not used in DECORD.
    ...
    #    return _CLASS_MODULE(ctypes_handle(value.v_handle))
        ...
        return fobj
    elif tcode in _DECORD_EXT_RET:
        return _DECORD_EXT_RET[tcode](ctypes_handle(value.v_handle))
...
cdef inline int FuncCall3(void* chandle,
    ...
    cdef DECORDValue[3] values
    cdef int[3] tcodes
    nargs = len(args)
    temp_args = []
    for i in range(nargs):
        make_arg(args[i], &values[i], &tcodes[i], temp_args)
    CALL(DECORDFuncCall(chandle, &values[0], &tcodes[0],
...
cdef inline int FuncCall(void* chandle,
    ...
    cdef vector[DECORDValue] values
...
cdef class FunctionBase:
    ...
    def __dealloc__(self):
        if self.is_global == 0:
            CALL(DECORDFuncFree(self.chandle))
