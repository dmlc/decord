...
cdef void _c_dlpack_deleter(object pycaps):
    ...
    if pycapsule.PyCapsule_IsValid(pycaps, _c_str_dltensor):
        ...
        DECORDDLManagedTensorCallDeleter(dltensor)
...
def _from_dlpack(object dltensor):
    ...
    if pycapsule.PyCapsule_IsValid(dltensor, _c_str_dltensor):
        ...
        CALL(DECORDArrayFromDLPack(ptr, &chandle))
...
cdef class NDArrayBase:
    ...
    property handle:
        def __get__(self):
            if self.chandle == NULL:
                ...
                return ctypes.cast(
                    <unsigned long long>self.chandle, DECORDArrayHandle)
    ...
    def __dealloc__(self):
        if self.c_is_view == 0:
            CALL(DECORDArrayFree(self.chandle))
    ...
    def to_dlpack(self):
        ...
        CALL(DECORDArrayToDLPack(self.chandle, &dltensor))
...
cdef _DECORD_COMPATS = ()

cdef _DECORD_EXT_RET = {}

def _reg_extension(cls, fcreate):
    global _DECORD_COMPATS
    _DECORD_COMPATS += (cls,)
    if fcreate:
        _DECORD_EXT_RET[cls._decord_tcode] = fcreate
