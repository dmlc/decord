 ...
 * \file pack_args.h
 * \brief Utility to pack DECORDArgs to other type-erased fution calling convention.
 *
 ...
 */
#ifndef DECORD_RUNTIME_PACK_ARGS_H_
#define DECORD_RUNTIME_PACK_ARGS_H_
 ...
 */
template<typename F>
inline PackedFunc PackFuncVoidAddr(F f, const std::vector<DECORDType>& arg_types);
/*!
 ...
 */
template<typename F>
inline PackedFunc PackFuncNonBufferArg(F f, const std::vector<DECORDType>& arg_types);
/*!
 * \brief Create a packed function that from function that takes a packed arguments.
 *
 * \param f with signature (DECORDArgs args, DECORDRetValue* rv, void* pack_args, size_t nbytes)
 ...
 */
template<typename F>
inline PackedFunc PackFuncPackedArg(F f, const std::vector<DECORDType>& arg_types);
/*!
 ...
 */
inline size_t NumBufferArgs(const std::vector<DECORDType>& arg_types);
...
inline PackedFunc PackFuncVoidAddr_(F f, const std::vector<ArgConvertCode>& codes) {
  ...
  auto ret = [f, codes, num_args](DECORDArgs args, DECORDRetValue* ret) {
...
inline PackedFunc PackFuncNonBufferArg_(
  ...
  int num_args = static_cast<int>(codes.size());
  auto ret = [f, codes, base, num_args](DECORDArgs args, DECORDRetValue* ret) {
    TempArray<ArgUnion, N> holder_(num_args);
    ArgUnion* holder = holder_.data();
    for (int i = 0; i < num_args; ++i) {
      switch (codes[i]) {
...
inline PackedFunc PackFuncPackedArg_(
  ...
  auto ret = [f, codes, num_args](DECORDArgs args, DECORDRetValue* ret) {
    ...
    int32_t* ptr = pack;
    static_assert(sizeof(DECORDValue) == 8, "invariant");
    ...
    for (int i = 0; i < num_args; ++i) {
      switch (codes[i]) {
        ...
        case INT64_TO_INT64:
        case FLOAT64_TO_FLOAT64: {
          std::memcpy(ptr, &args.values[i], sizeof(DECORDValue));
          ptr += 2;
          break;
        }
        case INT64_TO_INT32: {
...
#endif  // DECORD_RUNTIME_PACK_ARGS_H_
