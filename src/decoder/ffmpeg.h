...
struct AVPacket_ {
    ...
    static void Deleter(AVPacket_ *self) { if (!self->ptr) return; AVPacket *p = self->Get(); av_packet_free(&p); self->ptr = nullptr; }
...
struct AVFrame_ {
    ...
    static void Deleter(AVFrame_ *self) { if (!self->ptr) return; AVFrame *p = self->ptr.get(); av_frame_free(&p); self->ptr = nullptr; }
    DLTensor ToDLTensor();
...
struct AVCodecContext_ {
    ...
    AVCodecContext_(AVCodecContext *p) { ptr = std::shared_ptr<AVCodecContext>(p, &AVCodecContext_::Deleter); }
    AVCodecContext* Get() { return ptr.get(); }
    static void Deleter(AVCodecContext_ *self) { if (!self->ptr) return; AVCodecContext *p = self->ptr.get(); avcodec_free_context(&p); self->ptr = nullptr; }
...
struct SwsContext_ {
    ...
    SwsContext_(SwsContext *p) { ptr = std::shared_ptr<SwsContext>(p, &SwsContext_::Deleter); }
...
class FFMPEGThreadedDecoder {
    ...
    private:
        static void WorkerThread(PacketQueuePtr pkt_queue, FrameQueuePtr frame_queue, 
